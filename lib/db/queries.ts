...
const DEFAULT_CATEGORIES: {
  name: string;
  type: "utility" | "debt" | "savings" | "misc";
}[] = [
...
export async function getSavingsAccounts(userId: string) {
  return db
    ...
    .where(
      and(eq(savingsAccounts.userId, userId), eq(savingsAccounts.isArchived, false))
    )
...
export async function createSavingsAccount(
  userId: string,
  data: { name: string; initialBalance?: string; targetAmount?: string }
) {
  const [created] = await db
...
export async function updateSavingsAccount(
  ...
  const [updated] = await db
    ...
    .set(data)
    .where(
      and(eq(savingsAccounts.id, id), eq(savingsAccounts.userId, userId))
    )
    .returning();
  return updated ?? null;
}
...
export async function getGoalAccounts(userId: string, goalId: string) {
  const rows = await db
    ...
    .where(
      and(eq(goals.userId, userId), eq(goalAccounts.goalId, goalId))
    );
